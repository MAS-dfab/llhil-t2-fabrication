...
class ScrewSolver:
    ...
    def shrink_aligned_entry_corners(self, joint, angle):
        ...
        amp_end = spec.SCREW_DIAMETER / math.sin(
            angle_dire_main
        )  # The screw will attach the edge if divided by 2.0
    ...
    def calculate_screw_capacity(self, joint, angle=None):
        ...
        if joint.entry_type == "aligned":
            ...
            table = sorted(
                spec.spec_table["min_widths"].items(), key=lambda x: x[0], reverse=True
            )
            max_l_num = next(
                (pair_count for pair_count, min_width in table if len_l >= min_width), 0
            )

        elif joint.entry_type == "krossed":
            spec = self._spec_cache["krossed"]
            entry_corners_a, _ = joint._calculate_entry_exit_frames(
                angle=angle, data_type="points"
            )["sides"][0]
            entry_corners_b, _ = joint._calculate_entry_exit_frames(
                angle=angle, data_type="points"
            )["sides"][1]
            entry_bottom, _ = joint._calculate_entry_exit_frames(
                angle=angle, data_type="points"
            )["bottom"]
            ...
            len_l = (
                len_l_a if len_l_a < len_l_b else len_l_b
            )  # Take smaller value of the two sides. or average?
    ...
    def populate_aligned_entry_points(self, joint, angle, amount: int = None):
        ...
        if amount is None:
            ...
            )  # Sort by length descending, then width descending
            best = next(
                ((w_num, l_num) for w_num, l_num in distributions if l_num <= max_l_num)
            )
    ...
    def regenerate_aligned_entry_points(self, joint):
        ...
        if not self.capacity_warnings:
            return
        warning = next(
            (w for w in self.capacity_warnings if w["joint_guid"] == joint.guid), None
        )
    ...
    def populate_krossed_entry_points(self, joint, angle, amount: int = None):
        ...
        corners_a, _ = joint._calculate_entry_exit_frames(
            angle=angle, data_type="points"
        )["sides"][0]
        ...
        corners_b, _ = joint._calculate_entry_exit_frames(
            angle=angle, data_type="points"
        )["sides"][1]
    ...
    def generate_screw(
        ...
        if not valid_screws:
            ...
            if (spec.PENENTRATION_ENTRY + spec.PENENTRATION_EXIT) > screw.length:
                ...
                screw.reject_reason = (
                    RejectReason.ENTRY_MATERIAL_TOO_THICK
                )  # Do counterbore
            return screw

        # 3.
        max_allowed_length = depth - spec.BACK_THRESHOLD
        suitable_length = next(
            (l for l in valid_screws if l <= max_allowed_length), None
        )
        if suitable_length is None:
            screw.is_valid = False
            screw.length = valid_screws[-1]
            screw.reject_reason = (
                RejectReason.EXIT_PROTRUSION
            )  # Protuding out of the exit face
            screw.dist_in_exit = screw.length - dist_entry
            return screw
        ...
        for i, row in enumerate(point_grid):
            for j, pt_entry in enumerate(row):
                screw = self.generate_screw(
                    pt_entry, dire, interface, exit_frame, spec, screw_lengths
                )
    ...
    def populate_crossed_screws(self, joint, point_grid):
        ...
        interface = joint.get_interface_boundary(data_type="frame")
        exit_frames = [
            joint.cross_beam.ref_sides[(joint.cross_beam_ref_side_index + i) % 4]
            for i in range(1, 4)
        ]
        ...
        for i, row in enumerate(point_grid):
            for j, pt_entry in enumerate(row):
                ...
                side_vec = dire.rotated(
                    sign * math.radians(spec.side_angle), rotate_axis
                )
                ...
                screw = self.generate_screw(
                    pt_entry, side_vec, interface, exit_frame, spec, screw_lengths
                )
                screw.joint_guid = joint.guid
                screw.joint_type = joint.__class__.__name__
                screw.position = (i, j)
    ...
    def populate_krossed_screws(self, joint, angle, point_grid):
        ...
        frames_data = joint._calculate_entry_exit_frames(
            angle=angle, data_type="points"
        )
        ...
        for si, side in enumerate(sides_info):
            ...
            if pts_exit and len(pts_exit) >= 3:
                exit_frames.append(
                    Frame.from_points(pts_exit[0], pts_exit[1], pts_exit[2])
                )
            else:
                exit_frames.append(None)
            base_dirs.append(
                screw_dirs_sides[si] if si < len(screw_dirs_sides) else None
            )
        ...
        if pts_exit_bottom and len(pts_exit_bottom) >= 3:
            exit_frames.append(
                Frame.from_points(
                    pts_exit_bottom[0], pts_exit_bottom[1], pts_exit_bottom[2]
                )
            )
        else:
            exit_frames.append(None)
        ...
        for i, row in enumerate(point_grid):
            ...
            for j, pt_entry in enumerate(row):
                ...
                if base_dir is None or exit_frame is None:
                    screw = Screw(
                        pt_entry, base_dir or Vector(0, 0, 1), spec.SCREW_DIAMETER
                    )
                    screw.is_valid = False
                    screw.reject_reason = RejectReason.EXIT_MATERIAL_TOO_THIN
                    screw.joint_guid = joint.guid
                ...
                screw = self.generate_screw(
                    pt_entry, actual_dir, interface, exit_frame, spec, screw_lengths
                )
                screw.joint_guid = joint.guid
                screw.joint_type = joint.__class__.__name__
                screw.position = (i, j)
    ...
    def add_drilling_features(
        self, joint, screws, target="main", depth_limited=True, tol=1e-1
    ):
        """
...
def apply_screws(
    ...
    """
    # if add_features:
    # model.process_joinery() #! this is a duplicate call, resulting extensions being added twice.
    ...
    for joint in joints_to_process:
        ...
        if joint.entry_type == "aligned":
            success, entry_point_grid = solver.populate_aligned_entry_points(
                joint, angle=screw_angle, amount=screw_amount
            )
            if not success:
                entry_point_grid = solver.regenerate_aligned_entry_points(joint)
            screws = solver.populate_aligned_screws(
                joint, angle=screw_angle, point_grid=entry_point_grid
            )

        elif joint.entry_type == "crossed":
            entry_point_grid = solver.populate_crossed_entry_points(
                joint, amount=screw_amount
            )
            screws = solver.populate_crossed_screws(joint, entry_point_grid)

        elif joint.entry_type == "krossed":
            entry_points = solver.populate_krossed_entry_points(
            ...
            screws = solver.populate_krossed_screws(
        ...
        for screw in screws:
            ...
            if screw.reject_reason == RejectReason.EXIT_PROTRUSION:
                ...
                counterbored_lines = screw.counterbore(
                    penetration=solver.get_specification(joint).PENENTRATION_EXIT
                )

        if add_features:
            solver.add_drilling_features(
                joint, screws=screws, target=drill_target, depth_limited=depth_limited
            )
        ...
        if with_data:
            if joint.name == "KBirdsmouthJoint":
                ...
                interface = joint.get_interface_boundary(data_type="polyline")
                entry_face, exit_face = joint.find_screw_boundaries(
                    angle=screw_angle, data_type="polylines"
                )

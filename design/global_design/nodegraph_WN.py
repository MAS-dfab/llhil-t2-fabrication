...
class NodeGraph(Graph):
    ...
    def add_segment_perpendicular(self, x, u, v, **attr):
        """
        Create a node at the perpendicular foot from node x onto line u-v,
        then connect x to that node and reconnect u/v's children.
        
        Unlike add_segment (which uses midpoint t=0.5), this finds the
        closest point on the u-v line segment to node x. For asymmetric
        structures, this ensures the new node aligns with x's axis.
        
        Parameters
        ----------
        x : int
            Node key to connect from.
        u, v : int
            Node keys defining the line segment to project onto.
        **attr
            Attributes for the new node.
        ...
        -------
        int or None
            New node key, or None if nodes don't exist.
        """
        if not (self.has_node(x) and self.has_node(u) and self.has_node(v)):
            return None

        px = self.node_attribute(x, "point")
        pu = self.node_attribute(u, "point")
        pv = self.node_attribute(v, "point")

        # Compute perpendicular foot parameter t on line u→v
        # foot = pu + t * (pv - pu), where t = dot(px-pu, pv-pu) / dot(pv-pu, pv-pu)
        dx = pv.x - pu.x
        dy = pv.y - pu.y
        dz = pv.z - pu.z
        ...
        t = ((px.x - pu.x) * dx + (px.y - pu.y) * dy + (px.z - pu.z) * dz) / len_sq
        ...
        if "mobility" not in attr:
            attr["mobility"] = "yz_free"
        if "group" not in attr:
            attr["group"] = self.node_attribute(u, "group")
        if "level" not in attr:
            attr["level"] = self.node_attribute(u, "level")
        ...
        w = self.get_or_add_point_node(foot, **attr)

        # Get children before potentially deleting nodes
        children_u = self.node_attribute(u, "children") or []
        children_v = self.node_attribute(v, "children") or []

        if children_u or children_v:
            candidates = set(children_u + children_v) - {x, u, v, w}
            ...
            self.add_graph_edge(x, w)

            for n in candidates:
                if self.has_node(n):
                    self.add_graph_edge(n, w)
        else:
            self.add_graph_edge(x, w)

        return w

    def add_segments_perpendicular(self, pairs):
        """
        Apply add_segment or add_segment_perpendicular based on tuple length.
        
        - 2-tuple (u, v): direct edge (same as add_segment)
        - 3-tuple (x, u, v): perpendicular foot from x onto u-v
        ...
        for pair in pairs:
            if len(pair) == 2:
                ...
                results.append(self.add_segment_perpendicular(*pair))
        return results

### Edited by Jerry on 31 Mar 2026
...
class VertexList:
    ...
    def delete_pairs(self, tuples):
        """
        Delete pairs by a list of tuples (start_idx, end_idx). Does not need to consider level, CrossSection and BeamCategory.
        """
    ...
    def move(self, indices, vectors):
        """Bugs: vertices change in place every time move is called. 31 Mar 2026."""
        ...
        for i in range(div_z):
            ...
            for j in range(next_div_x):
                for k in range(next_div_y):
                    ...
                    if i == 0:
                        categories = [BeamCategory.SINGLE, BeamCategory.CLT_ATTACHED]
                    else:
                        categories = [BeamCategory.SINGLE]
    
                    self.pairs.append(Pair(idx1, target_idx, level=i, cross_section=None, categories=categories))
                    self.pairs.append(Pair(idx2, target_idx, level=i, cross_section=None, categories=categories))
                    self.pairs.append(Pair(idx3, target_idx, level=i, cross_section=None, categories=categories))
                    self.pairs.append(Pair(idx4, target_idx, level=i, cross_section=None, categories=categories))
            curr_div_x -= 1
            curr_div_y -= 1
    ...
    def set_default_height(self, height_list):
        ...
        Args:
            height_list (list of float)(optional): A list of heights representing the point.z values for each level.
            If not provided, the height will be divided equally.
            The length of height_list should be equal to (division_x + 1) if provided.
        """
    ...
    def set_default_roof(self, mesh):
        ...
        Args:
            mesh (Mesh): A mesh representing the CLT panels on the roof.
        """
    ...
    def facade_points(self):
        ...
        return [self._default_vertices[idx] for idx in facade_indices]
    ...
    def roof_points(self):
        ...
        max_indices = (self.div_x + 1) * (self.div_y + 1)
        return self._default_vertices[:max_indices]
...
class BeamList:
    ...
    def double(self, indices):
        ...
        for idx in indices:
            ...
            # 5. Add short beams between double beams
    ...
    def group_by_module(self, polylines, tol=1e-3, optional_indices=[]):
        """
        ...
        if not optional_indices:
            ...
            beams = [self.beams[idx] for idx in optional_indices]
    ...
    def rotate_xy(self, angle):
        ...
        Args:
            ...
            point (Point): Rotation base point.
        ...
        pts = []
        for beam in self.beams:
            if beam is None or BeamCategory.SINGLE not in beam.categories:
                continue
            
            axis = Vector(0, 0, 1)
            pt = beam.mid

            new_start = beam.start.rotated(angle, axis, pt)
            new_end = beam.end.rotated(angle, axis, pt)
            # beam.start = new_start
            # beam.end = new_end

            # beam.start.rotate(angle, axis, pt)
            # beam.end.rotate(angle, axis, pt)
            pts.append((new_start, new_end))
        return pts
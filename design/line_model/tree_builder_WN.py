...
def boundary_from_breps_projection(input_breps, tolerance=0.01):
    """
    Create a boundary curve as the bounding rectangle of projected Breps.
    
    Always returns a 4-corner rectangle from the combined bounding box,
    avoiding issues with curved edges (ellipse arcs, etc.) that
    curve_to_compas_polyline cannot handle.
    """
    if not input_breps:
    ...
    # Always use bounding box rectangle (handles all brep shapes uniformly)
